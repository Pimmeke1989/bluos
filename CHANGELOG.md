...
### Removed
- **Group Name Parameter**: Removed non-functional `group_name` parameter from `bluos.join` service
  - The parameter was not working correctly with the BluOS API
  - BluOS automatically generates group names (e.g., "Woonkamer Stereo+FLEX Speaker")
  - Removed from service definition, registration, and API calls
  - **Breaking Change**: If you were using `group_name` in automations, remove it (it wasn't working anyway)

### Changed
- **Join Service**: Simplified to only require `master` parameter
  - Group names are now automatically generated by BluOS
  - Visible in `group_name` attribute after joining

### Technical Details
- Removed `group_name` parameter from `add_slave()` method in bluos_api.py
- Removed `group_name` parameter from `async_join_player()` method in media_player.py
- Removed `group_name` field from services.yaml and strings.json
- BluOS generates group names in format: "{Master Name}+{Slave Name}"
